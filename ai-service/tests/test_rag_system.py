...
"""
import unittest
from openclaw.rag_system import RAGSystem
...
class TestRAGSystem(unittest.TestCase):
    ...
    def test_retrieve_reverse_match(self):
        """测试反向匹配（查询词在内容中）"""
        results = self.rag_system.retrieve("云南省教育厅")
    ...
    def test_retrieve_top_k(self):
        """测试返回指定数量的结果"""
        results = self.rag_system.retrieve("高中", top_k=2)
        
        self.assertEqual(len(results), 2)
        
        results_5 = self.rag_system.retrieve("高中", top_k=5)
